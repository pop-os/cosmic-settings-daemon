    ...
    loop {
        tokio::select! {
            ...
            result = state_changed_stream.next() => {
                ...
                // Silence the critical nag as soon as the charger is connected,
                // rather than waiting to reach 10% or for an acpid AC-plug event
                // that never arrives on some hardware. UPower's State is the
                // authoritative source for whether we are actually discharging.
                ...
                if let Ok(new_percent) = message.get().await {
                    match new_percent {
                        percent if percent < 10.0 => {
                            // Base the nag on UPower's actual charging state, not on
                            // acpid AC-plug events (unreliable on some hardware):
                            // otherwise the alarm keeps sounding while charging until
                            // the battery climbs back above 10%. (cosmic-epoch#1279)
                            ...
                            if current_battery == BatteryLevel::Critical {
                                // Re-evaluate every tick so charging silences the nag.
